...
$cid=-1;  
 if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        $alert_id = $row["id"]; 
        if( $row["challenge_id"] != $cid) { 
            $cid = $row["challenge_id"];
            $chall = new challenge($cid,$conn);
            $finals = $chall->createTable();
            $plassering=0;
            
        }
        $currentUser = new user( intval($row["athlete_id"]) );
        $last_pos =  $row["pos"];
        $new_pos = "";
        foreach($finals as $item) {
            $plassering++;
           
            //if( intval($item["id"]) == $currentUser->id)   echo $item["total"] . "<br>";
            if( intval($item["id"]) == $currentUser->id  && $item["total"] != "") {
                if( $last_pos == "-") $last_pos = 99999999;
                //echo $plassering ."<". intval($last_pos) . "<br>";
                if( $plassering< intval($last_pos) ) {
                    $new_pos =$plassering;
                   $change = "up";  
                    break;
                } else if( $plassering>intval($last_pos) ) {
                    $new_pos =$plassering;
                    $change = "down";
                    break;
        ...
        if($change != "none") {
             ...
             $sql = "UPDATE email_alert SET pos = $new_pos WHERE id=$alert_id";
            if ($conn->query($sql) === TRUE) {
                    echo "Record updated successfully : ". $alert_id . "<br>";
            } else {
            ...
            // the message
            if($change == "up") $msg = "<strong>Gratulerer du klatrer på tabellen på " . $chall->name . "!</strong><br>Du ligger nå på en " . $plassering .". plass" ;
            else $msg = "<strong>Aiai, du rykker nedover på tabellen på " . $chall->name . "!</strong><br>Du ligger nå på en " . $plassering .". plass" ;

       
            echo $row["email"] . " : " . $msg . "<br><br>";
            
            $path = "http://segmentchallenge.lopsloggen.no/challenge/" . $chall->path;
            $to = $row["email"];
            $subject = "Varsel fra segmentChallenge";
            $html =  emailTemplate($msg,$subject,$path);
            ...
            // More headers
            $headers .= 'From: <webmaster@lopsloggen.no>' . "\r\n";
        ...
        echo $alert_id . ": ingen endringer, ingen epost...<br>";
        }
...
<?php 
function emailTemplate($msg,$subject,$path) {ob_start(); ?>
                                    ...
                                    <table border="0" cellpadding="0" cellspacing="0" width="100%" id="templatePreheader">
                                        <tr>
                                            <td valign="top" class="preheaderContent" style="padding-top:10px; padding-right:20px; padding-bottom:10px; padding-left:20px;" mc:edit="preheader_content00">
                                                Varsle om ny plassering på en segmentChallenge!
                                            </td>
                                            <!-- *|IFNOT:ARCHIVE_PAGE|* 
                                    ...
                                    <table border="0" cellpadding="0" cellspacing="0" width="100%" id="templateHeader">
                                        <tr>
                                            <td valign="top" class="headerContent">
                                            	...
                                            	<img src="http://segmentchallenge.lopsloggen.no/images/mail_blue_sky_by_myrzik137.jpg" style="max-width:600px;" id="headerImage" mc:label="header_image" mc:edit="header_image" mc:allowdesigner mc:allowtext />
                                            </td>
                                        </tr>
                                    </table>
                                    <!-- // END HEADER -->
                                    ...
                                    <table border="0" cellpadding="0" cellspacing="0" width="100%" id="templateBody">
                                        <tr>
                                            <td valign="top" class="bodyContent" mc:edit="body_content00">
                                                <h1>Varsel fra segmentChallenge!</h1>
                                                <?php echo $msg; ?>
                                                <p><a href='<?php echo $path;?>'>Se hele tabellen her</a></p>
                                            </td>
                                        </tr>
                                    	<tr>
                                        	<td class="bodyContent" style="padding-top:0; padding-bottom:0;">
                                            	<img src="http://segmentchallenge.lopsloggen.no/segmentchallenge.png" style="max-width:560px;" id="bodyImage" mc:label="body_image" mc:edit="body_image" mc:allowtext />
                                            </td>
                                        </tr>
                                        <tr>
                                    ...
                                    <table border="0" cellpadding="0" cellspacing="0" width="100%" id="templateFooter">
                                        ...
                                        <tr>
                                            <td valign="top" class="footerContent" style="padding-top:0;" mc:edit="footer_content02">
                                            	<i>Hilsen Admin</i>
                                <p><small>ps. du har skrudd på varsler på tabellendringer, hvis du ikke vil ha varsler fra denne siden, skru av <a href='<?php echo $path; ?>'>her</a></small></p>
                                            </td>
                                        </tr>
                                    </table>
                                    <!-- // END FOOTER -->
